...
begin
   update EMP
   set COMM = case
             when SAL > 1800 then nvl(COMM,0)+100
             when SAL between 1700 and 1800 then nvl(COMM,0)+50
             Else nvl(COMM,0)+30
             end
             where EMPNO =&prmEMPNO;
 end;
 SELECT * FROM EMP;
...
V_dept dept%rowtype;
begin
V_dept.deptno :=prmdeptno;
V_deptdname :=prmlc,
insert into dert values (v.deptdeptno,v.dept.dname,vdept.loc);
end;
